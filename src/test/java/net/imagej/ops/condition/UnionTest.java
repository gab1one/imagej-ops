...
public class UnionTest<T> extends AbstractOpTest {
	...
	public void testUnion() {
		...
		Condition<?> c1 = (Condition<?>) ops.op(FunctionGreaterCondition.class,
				5.0, 3.0);
		Condition<?> c2 = (Condition<?>) ops.op(FunctionGreaterCondition.class,
				5.0, 4.0);

		condition.add((Condition<T>) c1);
		condition.add((Condition<T>) c2);

		Boolean result = (Boolean) ops.run(UnionCondition.class,
				condition);
		assertSame(result, true);

		condition.add(0, (Condition<T>) c2);
		Boolean result1 = (Boolean) ops.run(UnionCondition.class,
				condition);
